    ...
    <section id="blog" className="section-padding border-b border-ink-200">
      <div className="container-max">
        <div className="flex items-end justify-between mb-12">
          <SectionHeader label="Writing" title="Blog" subtitle="Notes on mathematics, computing, and learning." />
          <Link href="/blog" className="hidden md:inline-flex items-center gap-1.5 text-xs font-mono text-accent hover:underline mb-4">
          ...
          {blogPosts.slice(0, 3).map((post, i) => (
            <Link key={post.slug} href={`/blog/${post.slug}`}
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-ink-500 line-clamp-1">{post.excerpt}</p>
              </div>
              <div className="flex sm:flex-col items-center sm:items-end gap-3 sm:gap-1 flex-shrink-0">
                ...
                <span className="font-mono text-xs text-ink-400">{post.readingTime}</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="mt-5 md:hidden">
          <Link href="/blog" className="inline-flex items-center gap-1.5 text-xs font-mono text-accent hover:underline">
            All posts <ArrowRight size={13} />
          </Link>
        </div>
      </div>
