...
export default function CertificatesSection() {
  return (
    <section id="certificates" className="section-padding border-b border-ink-200 bg-white">
      <div className="container-max">
        <SectionHeader label="Credentials" title="Certificates" />
        <div className="grid sm:grid-cols-2 gap-4">
          {certificates.map((c, i) => (
            <div key={i} className="flex gap-4 items-start p-5 border border-ink-200 rounded-xl hover:border-accent/40 transition-colors group">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs text-ink-400">{c.year}</span>
                  {c.url && (
